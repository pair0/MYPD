...
router.post("/login", async function (req, res) { //로그인 신청
    ...
    if (result == 0) {
        res.send(
            `<script>alert('아이디 혹은 패스워드가 잘못되었습니다.');location.replace("/user/login")</script>`
        );
    } else {
        bcrypt.compare(pw, result[0].e_customer_pw, (err, same) => {
            if (!same) {
                res.send(
                    `<script>alert('아이디 혹은 패스워드가 잘못되었습니다.2');location.replace("/user/login")</script>`
                );
            } else {
                var username = result[0].e_customer_id;
                res.send(
                    `<script>alert('로그인 성공! ${username}님 안녕하세요!');location.replace("../../main/")</script>`
                );
            }
        })
    }
...
router.post('/join', [
    ...
    body('pw_check').custom((value, {req, res, path}) => {
        if (value !== req.body.pw) {
            // return res.send(`<script>alert('악성 사용자 꺼저라 시발련아!
            // ');location.replace("../../user/join")</script>`);
            return res
                .status(400)
    ...
    bcrypt.hash(req.body.pw, 10, function (err, hash) {
        if (err) 
            return next(err)
        info['pw'] = hash;
        var sql = 'INSERT INTO Customers_Enterprise(enterprise_number, e_customer_id, e_customer_' +
                'pw, e_customer_email) VALUES(?,?,?,?)';
        var params = [
            info['number'], info['id'], info['pw'], info['email']
        ];
        ...
        res.send(
            "<script>alert('회원가입 신청 완료되었습니다.!! 승인 완료 시 해당 이메일로 승인 메일이 발송됩니다.');location.hre" +
            "f='/user/login';</script>"
        );
    })
...
router.post("/check_all", function (req, res, next) { //회원가입 검증
    const {checkID, checkPW, comparePW, checkMAIL} = req.body;
...
router.post("/mail_send", async function (req, res, next) {
    const mail = req.body.mail;

    number = await emailsend
        .sendmail(toEmail = mail)
        .catch(console.error);
    if (number) {
...
router.post("/mail_check", function (req, res, next) {
    mail_check = req.body.mail_check;
    var hashAuth;
    if (req.cookies.hashAuth) 
        hashAuth = req.cookies.hashAuth;
    try {
